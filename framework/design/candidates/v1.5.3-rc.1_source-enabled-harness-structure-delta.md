...
다른 프로젝트에는 Framework Standard Harness를 그대로 복사하고 프로젝트 차이만 Overlay한다.

```mermaid
flowchart LR
    C["Common Rule/Skill/Semantic Template"] --> P["Preset"] --> R["Project Profile"] --> O["Project Overlay"] --> D["Domain Overlay"]
...
v1.5.1은 Rule/Skill/Template/Overlay의 설계 계약을 정의했지만 당시 Repository에는 `.cursor/rules`, `.cursor/skills`, `sdlc/templates`가 실제 이식 가능한 Package로 materialize되어 있지 않았다. Source가 연결된 프로젝트에서 DISCOVERY 이후 어떤 Evidence를 Skill이 읽고 어떤 Template에 남겨야 하는지도 자동 검증되지 않았다.

### GAP-HS-001 Portable Harness Package Materialization
- 유형: Implementation Gap + Contract Gap
- 조치: 공통 Rule/Skill/Semantic Template 실제 파일 구조 생성
...
- 조치: Framework Standard를 수정하지 않고 Project/Domain 차이만 관리하는 Skeleton과 precedence 고정

### GAP-HS-005 Runtime Overlay Resolution
- 유형: Implementation Gap
- 상태: DESIGNED_NOT_IMPLEMENTED
- 조치: 이 Candidate에서는 적용 순서와 폴더/계약까지만 고정하고 Template Section merge, Rule/Config materialization 같은 실제 Runtime resolver는 후속 PoC 대상으로 둔다.

## 2. 기존 설계

설계상 사용자 Skill은 `/work /change /check /setup`, `/work` 내부 Reference는 requirement~verify, Template는 Markdown 중심, Customization은 Common→Preset→Profile→Domain→Local 순이었다. 하지만 파일/검증 계약이 아직 충분히 구현되지 않았다.
...
- Template 파일명은 Runtime Artifact명이 아니라 Semantic Template 이름이므로 영문 고정명을 허용한다. 생성되는 사용자 문서는 기존 직관적 한글 suffix 규칙을 유지한다.
...
- Overlay Customization: ENHANCED (Skeleton/precedence/Validator 추가; Runtime resolver는 당시 후속 구현 대상)

## 8. Migration

1. 공통 `.cursor`와 `sdlc/templates/semantic`을 프로젝트에 배치한다.
...
- 모든 공통 Rule/Skill/Semantic Template 필수 파일 존재
- `/work` Reference 10종이 동일 Contract Section 보유
- DISCOVERY~VERIFY Template가 Evidence locator/confidence를 보유
- Project Overlay precedence가 고정되어 Framework Standard silent override 방지
...
이 문서는 v1.5.3-rc.1 Candidate 당시의 Delta 기록이다. 이후 버전에서 구현 상태가 변경될 수 있으므로 현재 상태 판정은 최신 Branch의 Runtime/Contract/Test/CI를 기준으로 한다.
