...
`OPEN 해소 기본 경로`는 **Agent가 근거를 먼저 추가 탐색 → 그래도 결정권한이 필요한 항목만 사람에게 확인 → 확인 결과로 초안을 갱신**하는 순서입니다.

SOP가 없다는 이유만으로 모든 질문을 사람에게 돌리지 않습니다. Source로 확인할 수 없는 업무정책, 범위, 권한, 승인만 사람에게 확인합니다.
