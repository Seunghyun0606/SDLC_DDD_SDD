...
SOP와 기존 설계서는 있으면 분석 정확도를 높이는 **선택 Evidence**입니다. 자료가 없으면 Agent는 그 영역을 `영향 없음`으로 단정하지 않고 Coverage Gap 또는 `확인 필요`로 남깁니다.
...
`OPEN 해소 기본 경로`는 **Agent가 Source·기존 문서·DB/Interface Evidence를 먼저 추가 탐색 → 그래도 판단권한이 필요한 항목만 사람에게 확인 → 확인 결과로 초안을 갱신**하는 순서입니다.

Source에서 관찰된 동작은 AS-IS 근거일 뿐 고객의 목표 정책으로 자동 확정하지 않습니다.
