...
def apply_delta(store: dict, delta: dict) -> tuple[dict,dict]:
    """Return (result, resulting_store). Conflicts/invalid deltas never mutate the store."""
    ...
    # Phase 1: entities. This removes low-level-agent dependence on operation ordering.
    for index,op in entity_ops:
        ...
        if existing.get("truth_status")=="CONFIRMED_BUSINESS" and op["evidence_class"] in SOURCE_DERIVED and changed:
            return ({
                "status":"CONFLICT","delta_id":delta_id,"store_revision":store["revision"],
                "conflicts":[_error(
    ...
    # Phase 2: provenance-only observations. They can attach to confirmed truth without changing it.
    ...
    # Phase 3: relations. Entity endpoints may have been created anywhere in this delta.
