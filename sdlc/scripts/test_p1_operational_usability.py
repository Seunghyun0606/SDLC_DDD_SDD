...
def main():
    # Authority index: one concept -> one path; validation history/guides remain non-authoritative.
    ...
    assert 'sdlc/design' in authority.get('non_authoritative_roots',[])
    assert 'sdlc/guides' in authority.get('non_authoritative_roots',[])

    # Bootstrap points to the decision authority instead of duplicating the decision list.
    ...
    with tempfile.TemporaryDirectory() as td:
        d=Path(td)
        # Project decision registry preserves OPEN, and turns configured side effects into scoped blockers.
        ...
        # Overlay known key applies only with provenance/lifecycle; unknown key and stale base are denied.
        ...
        # Worklist: MD -> Canonical+XLSX roundtrip, same-revision divergence -> conflict and no silent overwrite.
        ...
        # Knowledge: observed Business Rule cannot promote without explicit human confirmation; reviewed confirmation promotes and is idempotent.
