...
def _run_setup(args: list[str]) -> int:
    ...
    if isinstance(result, dict) and result.get("status") != "SETUP_FAILED":
        ...
        result["user_entrypoint"] = {"start_here": "docs/00_시작/START_HERE.md", "project_setup_guide": "docs/00_시작/02_PROJECT_설정가이드.md", "tailoring_guide": "docs/00_시작/03_TAILORING_설정가이드.md", "zero_to_one_intake": "CONNECTED_WITH_RQ_EXTRACTION_MANIFEST", "work_model": "CHANGE_LEVEL_SEMANTIC_POLICY"}
        result["next_commands"] = ["python sdlc/scripts/harness.py check --setup", "python sdlc/scripts/harness.py intake <requirement-file.xlsx>"]; result.pop("next_if_target_exists", None); result.pop("next_if_no_target", None)
