...
import yaml
PROVIDER_TYPES={"SOURCE","TEST","CANONICAL_REGISTRY","COMMAND_ROUTER"}; MODES={"AUTO","GREENFIELD","BROWNFIELD","HYBRID"}; WRITE_MODES={"READ_ONLY","READ_WRITE"}; PROVIDER_STATES={"AVAILABLE","DEGRADED","UNAVAILABLE","UNCONFIGURED","DISABLED"}; RESPONSE_STATUSES={"OK","PARTIAL","BLOCKED","ERROR"}; TRUTH={"GIVEN","OBSERVED","INFERRED","CONFIRMED","OPEN"}; CAPABILITY=re.compile(r"^[a-z][a-z0-9_-]*(?:\.[a-z][a-z0-9_-]*){1,}$")
...
def validate_registry(data):
 ...
 for i,p in enumerate(providers):
  ...
  if p.get("provider_type") not in PROVIDER_TYPES:add(e,"P06-005",f"providers[{i}].provider_type is invalid")
...
def validate_request(data,registry=None):
 ...
 if req.get("provider_type") not in PROVIDER_TYPES:add(e,"P06-022","provider_type is invalid")
...
def validate_response(data,request=None,registry=None):
 ...
 if registry:
  ...
  elif p.get("provider_state") not in {"AVAILABLE","DEGRADED"}:add(e,"P06-055","response came from unusable provider state")
 return e
def main():
