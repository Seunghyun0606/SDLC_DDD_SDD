...
def main():
 ...
 unsupported=deepcopy(brown); unsupported["requested_capabilities"]=["monitoring.metric.read"]; plan,errors=build_plan(reg,unsupported); assert plan["runtime_plan"]["status"]=="ACTION_REQUIRED"; assert plan["runtime_plan"]["open_items"][0]["reason"]=="MISSING_CAPABILITY"
 ...
 extension=deepcopy(reg); extension["registry"]["providers"].append({"provider_id":"deploy","provider_type":"DEPLOYMENT","enabled":True,"provider_state":"AVAILABLE","mode":"READ_WRITE","capabilities":["deployment.release.execute"]}); assert validate_registry(extension)==[]
 deploy=deepcopy(brown); deploy["requested_capabilities"]=["deployment.release.execute"]; plan,errors=build_plan(extension,deploy); assert plan["runtime_plan"]["status"]=="READY"; assert any(x["provider_type"]=="DEPLOYMENT" for x in plan["runtime_plan"]["resolved_providers"])
