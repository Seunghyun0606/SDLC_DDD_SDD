...
```mermaid
flowchart LR
    RQ --> FR[기능 분해] --> BR[업무규칙 후보] --> AC[완료기준] --> Q[질문/가정]
```
...
```mermaid
flowchart LR
    RQ --> E[Source/DB/문서 Evidence] --> C[영향 후보] --> B[Business] --> F[Functional] --> T[Technical]
```
...
```mermaid
flowchart LR
    AC --> TC[Test Case] --> TR[Test Result] --> VR[Verification Result] --> K[Knowledge Candidate]
```
