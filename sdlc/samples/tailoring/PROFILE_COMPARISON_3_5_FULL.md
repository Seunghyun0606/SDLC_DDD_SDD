...
- Canonical fingerprint: `sha256:53f2b05fff4ca4b2e76c352bfce19cc167b9e1c45692fd340aabeef4059f1154`
...
다음 값은 세 Profile에서 동일해야 한다.

1. Canonical target 및 fingerprint
2. Change Level
3. Runtime Stage sequence
4. `stage_preserved = true`
5. `projection_creates_business_truth = false`

Profile이 바꾸는 것은 Human Artifact의 **분할/통합 방식, Template, 출력경로**다. Stage와 Canonical Truth의 권한은 바꾸지 않는다.

## 판정 경계

- 이 Sample의 PASS는 `Stage 유지 + Canonical 동일 + Projection 무변조`에 대한 **구조 검증**이다.
