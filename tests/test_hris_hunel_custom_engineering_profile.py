...
class HrisHunelCustomEngineeringProfileTest(unittest.TestCase):
    def test_custom_profile_and_templates_exist(self):
        ...
        self.assertIn('projection_topology: PROFILE_PRIMARY_SET', text)
        self.assertIn('L1: CONCISE', text)
        self.assertIn('L2: CONCISE', text)
        self.assertIn('L3: STANDARD', text)
        self.assertIn('L5: FULL', text)
    ...
    def test_project_example_selects_engineering_profile_only(self):
        ...
        self.assertIn('minimum_level: "L1"', text)
        self.assertIn('target_levels:', text)
