...
> 일반 참여자는 Template 빈칸을 직접 작성하는 사람이 아니다. Agent가 먼저 조사·초안을 만들고, 사람은 필요한 질문에 답하고 생성된 문서를 Review한다.
...
문서에서 의미가 잘못됐으면 관련 Block을 지정해 `/change`를 요청한다.
...
- 사람 결정이 필요한 항목은 HITL 질문/Next Action으로 드러난다.
