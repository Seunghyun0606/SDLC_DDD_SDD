...
- Config 설정 순서는 `02_PROJECT_설정가이드.md`, 옵션별 정확한 역할과 허용값은 `02A_PROJECT_CONFIG_옵션_상세가이드.md`를 사용한다.
...
사용자는 자연어로 짧게 답하면 된다. 내부 Block ID를 알 필요는 없다. Agent가 필요하면 숨김 Stable Block과 연결한다.

예:

```text
질문: 월 마감 후 자동 재계산을 허용할까요?
...
Agent는 답을 추정하지 않고 같은 Question ID를 Machine-side Queue 상태에 유지하고, 사람 문서에는 **확인 필요사항 / 현재 확인값·제안 / 담당 / 개발 영향 / 다시 확인할 시점 / 상태**처럼 자연어로 보여준다.

Machine-side에서는 `SOURCE_BLOCK / ITERATE / ALERT`, `Recheck At` 같은 제어값을 유지할 수 있지만 일반 Projection 본문에 그대로 노출하는 것이 기본 UX는 아니다.

- 개발 전 확인 필요: 이 답에 의존하는 Source/Action만 제한
- 진행 가능·추후 보완: 큰 방향은 정해져 있어 다음 단계 진행 가능, 지정 경계에서 재확인
- 주의사항: 현재 실행은 가능하지만 계속 추적

사용자가 아직 모른다고 한 것을 `ASSUMED` Business Truth로 자동 확정하지 않는다.

## 6. 생성 문서를 수정하는 방법

오탈자를 제외하면 직접 파일을 수정하기보다 **Target/문서 + 바꾸려는 의미 구간 + 원하는 변경**을 Agent에게 말한다.
...
```text
PGM-001 Program Spec의 Source 근거 부분을
현재 Source 기준으로 다시 조사해줘.
```

```text
HRIS 작업지시서의 저장 권한 처리 부분만 다시 검토해줘.
```

```text
RQ-0042의 미확정 사항 중 월 마감 정책은 아직 미확정이니
PROGRAM 단계에서 다시 확인해줘.
```

Harness 관리자나 고급 사용자가 Stable Block ID를 알고 있다면 `[BLOCK:WU-BUSINESS-RULE]`처럼 명시해도 된다. Block ID는 선택사항이며 일반 사용자가 외워야 하는 인터페이스가 아니다.
...
| Queue 상태/재확인 시점 조정 | Queue metadata 갱신, Business Truth 변화 없음 |
| Queue 질문에 실제 답변 | 의미에 따라 `/change` 또는 `/work`, Canonical/Provenance 반영 |
| 오탈자/표현/레이아웃 | Projection-only |

문맥이 명확하면 Agent가 적용할 의미 Block을 내부적으로 찾고 진행한다. 여러 후보가 있을 때만 적용 구간을 짧게 질문한다.
...
Config의 각 Key가 Runtime Switch인지 Agent/문서 Context인지, 기본값과 허용값이 무엇인지는 `02A_PROJECT_CONFIG_옵션_상세가이드.md`를 본다.
...
문서에서 의미가 잘못됐으면 관련 의미 구간을 지정해 `/change`를 요청한다. 즉시 확정할 수 없으면 다음 합의/설계 단계의 재확인 시점을 지정해 Queue로 보류할 수 있다.
...
- 인수/테스트 기준
- 현재 DESIGN/PROGRAM에서 재확인해야 할 항목
...
예상 외 영향이 커지면 Change Level Escalation Evidence가 된다. Machine-side `SOURCE_BLOCK`으로 분류된 미확정 사항은 그 결정에 의존하는 Source/Action만 결정 전까지 수정하지 않는다.
...
- 테스트 전에 확인해야 할 미확정 사항

**확정 책임**
- Test Result 사실과 Coverage Gap
- 합격 기준이 업무적으로 불명확할 때 Agent 질문에 답변

TC 문서를 처음부터 작성하기보다 Agent가 인수 조건에서 만든 Test 초안을 Review한다.
...
- 고객이 업무 의미를 변경하거나 확정한 내용은 관련 Section을 지정해 `/change` 또는 Customer Decision Round-trip으로 Canonical Provenance/Decision에 반영한다.
- 고객에게 내부 Engineering Template을 채우도록 요구하지 않는다.
- 회의 중 즉시 결정하지 못한 사항은 Queue로 남겨 다음 합의/인수 경계에서 다시 확인할 수 있다.
- Customer 문서에는 Canonical ID/Relation/Provenance/Confidence 같은 Machine detail을 기본 노출하지 않는다.
...
- 일반 사용자에게 Stable Block ID나 Machine Guard Enum을 외우도록 요구하지 않는다.
...
- 지금 답하지 못한 항목은 재확인 시점과 함께 다음 Semantic Work로 carry-forward된다.
- 생성 문서 수정 시 문서의 의미 구간을 자연어로 지정해 Agent에게 지시할 수 있다.
